...
class Plugin implements PluginInterface, EventSubscriberInterface
{
    ...
    public function activate(Composer $composer, IOInterface $io): void
    {
        ...
        $validator->validate(
            $options,
            (object)['$ref' => 'file://' . dirname(__DIR__) . '/res/schema.json'],
        ...
        if (!Filesystem::isReadable($globalPath) || !is_writable($globalPath)) {
            ...
            $io->write(
                sprintf(
                    '<error>Path "%s" is not writable for "%s", plugin disabled.</error>',
                    $globalPath,
                    self::PACKAGE_NAME
                )
            );
        ...
        foreach (debug_backtrace() as $trace) {
            ...
            try {
                ...
                $this->autoloadGeneratorOptions = [
                    ...
                    'apcu-autoloader' => $input->getOption('apcu-autoloader-prefix') !== null
                        || $input->getOption('apcu-autoloader'),
                    'ignore-platform-reqs' => $input->getOption('ignore-platform-reqs')
                        ?: ($input->getOption('ignore-platform-req') ?: false),
                ];
        ...
        $event->getComposer()
            ...
            ->addListener(
                ...
                function (Event $event): void {
                    ...
                    $optimize = $this->autoloadGeneratorOptions['optimize-autoloader']
                        || $config->get('optimize-autoloader');
                    $authoritative = $this->autoloadGeneratorOptions['classmap-authoritative']
